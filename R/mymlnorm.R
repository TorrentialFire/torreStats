#' Maximum likelihood estimate for normal distributions
#'
#' @param x vector of data for which to calculate the maximum likelihood estiamtes of mu and sigma
#' @param mu vector of values of mu to evaluate
#' @param sig vector of values of sigma to evaluate
#' @param ... additional graphing parameters
#'
#' @return a named list containing x, the input vector of data, coords, the coordinates in the mu-sigma matrix for the mu and sigma estimates, and maxl, the maximum likelihood
#' @export
#'
#' @examples mymlnorm(x=c(10,12,13), mu=seq(10,14,l=1000), sig=seq(0.1,4,l=1000))
mymlnorm <- function(x, mu, sig, ...){  #x sample vector
  ...
  lfun <- function(x, m, p) {
    log(stats::dnorm(x, mean = m, sd = p))   # log lik for normal
  }
  ...
  mly = round(sqrt((n - 1) / n) * stats::sd(x), 2)

  #axis(1,at=c(0:20,mlx),labels=sort(c(0:20,mlx)))
  #axis(2,at=c(0:20,mly),labels=TRUE)
  graphics::abline(v = mean(x), lwd = 2, col = "Green")
  graphics::abline(h = sqrt((n - 1) / n) * stats::sd(x), lwd = 2, col = "Red")

  # Now find the estimates from the co-ords
  muest <- mu[coord[1]]
  sigest <- sig[coord[2]]

  graphics::abline(v = muest, h = sigest)
